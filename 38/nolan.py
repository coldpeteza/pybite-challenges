...
def get_movies():
    ...
    movies: List[ET.Element] = get_tree().findall('.//movie')
    for movie in movies:
...
def get_movie_longest_runtime():
    ...
    movies: List[ET.Element] = get_tree().findall('.//movie')
    for movie in movies:
        movie_runtimes[movie.get('runtime').split(' min')[0]] = movie.get('title')

    max_runtime = max(movie_runtimes.keys())
    return movie_runtimes[max_runtime]
